...
class Export_Lock_Seat(object):
    ...
    def execute(self):
        ...
        while True:
            ...
            for i in range(50):
                try:
                    seat_query = ticket_util.getSeat(third_from,seq_no=seq_no,third_id=third_id,seat_index=i,mode=1)
                    seat_info = seat_query['seat_no'][0]
                except:
                    continue
                phone_number = '15800807450'
                params = {"third_from":third_from,"seq_no":seq_no,"third_id":third_id,"log_id":'123456',"phone_number":phone_number,"seat_info":seat_info}
                base_url = '/ticket/seat/lockseat'
...
def run(third_from):
    case = Export_Lock_Seat(third_from)
    case.execute()
