...
class Lock_Seat_Press(object):
    ...
    def execute(self):
        ...
        while True:
            ...
            # redis模式
            if MODE == 1:
                if self.redis.llen(REDIS_CONF['ALL_SEQ_Q']+'_'+self.third_from) == 0:
                    #print 'Seq queue empty for parterner: ' + self.third_from
                    #break
                    time.sleep(5)
                    continue
                seq_string = self.redis.rpop(REDIS_CONF['ALL_SEQ_Q']+'_'+self.third_from)
            ...
            if seq_outbuy_time < time_threshold:
                continue
            # 调用座位图接口
            get_seat = ticket_util.getSeat(third_from,seq_no=seq_no,third_id=third_id,mode=2)
            if get_seat.has_key('error_no') and get_seat['error_no'] == 0:
                self.redis.incr(REDIS_CONF['SUC_NO']+'_seatinfo')
            else:
                self.redis.incr(REDIS_CONF['FAIL_NO']+'_seatinfo')
