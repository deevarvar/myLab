...
CALL_RESOLUTION_KEY = "CAMERA_SIZE"
...
class vt_statistics:
    ...
    def call_active(self, line):
        ...
        self.call_start_time = temp.pop(0)
...
class vt_list:
    ...
    def parse(self, line):
        ...
        elif CALL_RESOLUTION_KEY in line:
            if self.flag == 0:
                return
            elem = self.list[self.num - 1]
        ...
        try:
            with open(report_path, 'w', encoding= 'utf-8') as out_file:
                if (len(self.text.get(1.0, tkinter.END))  > 1):
                    print(self.text.get(1.0, tkinter.END), out_file)
